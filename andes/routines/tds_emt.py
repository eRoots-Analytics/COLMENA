...
def f_condition(mdl, device_idx, colmena_device =[4]):
    if device_idx not in colmena_device:
        return False
    if mdl.omega[device_idx]>0.9 and mdl.omega[device_idx]<1.1:
        return False
    return True
dict_changes = {}
dict_changes["GENROU"] = (f_condition, "M", lambda x: 13, lambda x: 130)



class TDS_emt(BaseRoutine):
    """
    ...
    def run(self, no_summary=False, **kwargs):
        ...
        while (system.dae.t - self.h < self.config.tf) and (not self.busted):
            ...
            if self.data_csv is None:
                colmena_status = self.colmena_step(dict_changes) 
    ...
    def emt_step(self):
        #EMT STEP during normal functioning
        system = self.system
        for name, mdl in system.models.items():
            is_emt = getattr(mdl, "emt")
            if is_emt is not None and is_emt:
                mdl.emt_step()
