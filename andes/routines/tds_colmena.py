...
class TDS_colmena(BaseRoutine):
    ...
    def run(self, no_summary=False, **kwargs):
        ...
        while (system.dae.t - self.h < self.config.tf) and (not self.busted):
            ...
            if self.data_csv is None:
                colmena_status = self.colmena_step(system.dae.t) 
    ...
    def colmena_step(self, colmena_msg, dae_t = 0):
        ...
        for mdl_name, mdl in system.models.items():
            ...
            if self.config.instruction_msg.get(mdl_name) is None:
                continue
            
            instructions = self.config.instructions
            param_change = self.config.param
            uid = self.config.device_uid
            f_condition = instructions[0]
            operative_cond_false = instructions[1]
            operative_cond_true = instructions[2]
            
            for i, idx in enumerate(mdl.idx.v):
                if i != uid:
                    continue
                condition = f_condition(mdl, idx, epsilon = 0.003, dae_t = dae_t)
                old_v = getattr(mdl, param_change).v[i]
                new_value = condition*operative_cond_true(old_v) +  (1-condition)*operative_cond_false(old_v)
                if condition:
                    uid = mdl.idx2uid(idx)
                    mdl.alter(param_change, idx = idx, value = 0.2)
                    #mdl.alter("gammap", idx = idx, value = 0.2)
                if new_value != old_v:
                    update_service = True
