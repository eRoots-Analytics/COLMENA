...
import numpy as np
import traceback
# import AndesApp.aux_function as aux
import matplotlib.pyplot as plt
import matplotlib

from src.config.config import Config
from andes import load, get_case
...
def load_simulation():
    ...
    try:
        ...
        system = load(case_file, setup=False)
        print(f"case_file is {case_file}")

        system.prepare() #, autogen_stale=True
        # system setup and run power flow
        system.setup()
        system.PFlow.run()
        # Run TDS with small time step
        system.TDS.config.tf = 0.01
        system.TDS.run()

        print(f"system area is {system.Area}")
        return jsonify({"message": "Simulation loaded successfully"}), 200

    except Exception as e:
        print("Error in loading the simulation.")
        traceback.print_exc()
...
#         app.config['await_start'] = True
#         n_redual = 4
#         system_ieee = load(case_file, setup=False)
#         print(f"case_file is {case_file}")
#         if data.get('redual') is False:
#             system = system_ieee
#             system.prepare(pycode_path=None, autogen_stale=True)
#             # print("TGOV1N available parameters:")
#             # print(list(system.TGOV1N.__dict__.keys()))
...
def run_step():
    ...
    try:
        data = request.get_json()
        delta_t = float(data.get('delta_t', 0.1))   # NOTE: Config 
        speed_factor = float(data.get('speed', 1))  # NOTE: Config 
