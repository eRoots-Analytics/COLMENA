...
class AgentControl(Service):
    ...
    class GridFormingRole(Role):
        @Metric('frequency')
        @KPI('max(deriv(agentcontrol_frequency[30s])) > 1')
    ...
    class Monitoring(Role):
        ...
        def behavior(self):
            ...
            time.sleep(0.3)
            time_sync = self.andes.get_dae_time()
            time_sync2 = self.andes.get_dae_time()

            if self.agent_id != 'genrou_23' or self.agent_id != 'area_1':
                return
            while time_sync !=  time_sync2 and time_sync >18:
                time.sleep(10)
                time_sync2 = self.andes.get_dae_time()
