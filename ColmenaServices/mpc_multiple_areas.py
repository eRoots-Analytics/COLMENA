    ...
    def initialize_dual_vars(self):
        ...
        for t in model.TimeHorizon:
            for area in model.other_areas:
                ...
                self.dual_vars_P[self.area, area, t] = 0
                self.dual_vars_diff[self.area, area, t] = 0
                self.dual_vars[area, self.area, t] = 0
                self.dual_vars_P[area, self.area, t] = 0
                self.dual_vars_diff[area, self.area, t] = 0
        return self.dual_vars

    def initialize_horizon(self, n_areas):
        model = self.model
        self.state_horizon = {}
        for t in model.TimeHorizon:
            for area1 in range(1, n_areas+1):
                for area2 in range(1, n_areas+1):
...
class AgentControl(Service):
    ...
    @Data(name = 'dual_vars_P', scope = 'grid_areas/id =.')
    @Data(name = 'dual_vars_diff', scope = 'grid_areas/id =.')
    ...
    class Distributed_MPC(Role):
        ...
        def behavior(self):
            ...
            if not self.initialized_decorators:
                ...
                self.state_horizon = self.agent.initialize_horizon(self.n_areas)
