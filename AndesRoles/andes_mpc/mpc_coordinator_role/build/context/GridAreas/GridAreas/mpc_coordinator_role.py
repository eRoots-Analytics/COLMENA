    ...
    def warm_start(self, state_horizon):
        for t in self.model.TimeHorizon:
            ...
            for area in self.model.other_areas:
                ...
                self.model.dual_vars[self.area, area, t] =   self.dual_vars.get((area, self.area, t),0)
...
class AgentControl(Service):
    ...
    class LayerOne(Role):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.iter = 0
            self.max_iter = 100
            self.data_read = self.theta_data
            self.data_write = self.theta_channel
        ...
        def behavior(self):
            ...
            while self.error > self.agent.tol and self.iter < self.max_iter:
                ...
                self.state_horizon = {tuple(map(int, key.split("_"))): val for key, val in self.state_horizon_jsonlike.items()}
                ...
                for t in self.model.TimeHorizon:
                    for area_i in self.model.other_areas:
                        ...
                        delta_2 = (self.state_horizon[area_i, self.area, t] - self.model.delta_areas[area_i, t].value)
                        self.dual_vars[area_i, self.area, t] += self.agent.rho*(self.model.delta[t].value - self.state_horizon[area_i, self.area, t])
                        self.dual_vars[self.area, area_i, t] += self.agent.rho*(self.state_horizon[area_i, self.area, t] - self.model.delta_areas[area_i, t].value)
                ...
                if track_error:
                    ...
                    while not changed_horizon: 
                        ...
                        print(f'Waiting 3 for {self.iter}')
                        for neighbor_area in self.neighbors:
                            area_changed = False
                            for t in self.model.TimeHorizon:
                                if state_horizon_jsonlike[f'{neighbor_area}_{self.area}_{t}'] != initial_state_horizon_jsonlike[f'{neighbor_area}_{self.area}_{t}']:
                                    area_changed = True
                            changed_horizon = (not area_changed) and changed_horizon
                        time.sleep(0.005)
                self.iter += 1
    ...
    class CoordinatorRole(Role):
        ...
        def behavior(self, new_theta):
            ...
            if self.first:
                for area1 in range(0, self.n_areas):
                    for area2 in range(0, self.n_areas):
                        if area1 != area2:
                            for t in range(self.T):
                                all_vars[area1, area2, t] = 0
                self.first = False

            for key,val in new_theta.items():
                all_vars[key] = val
            self.theta_data.publish(all_vars)
            return