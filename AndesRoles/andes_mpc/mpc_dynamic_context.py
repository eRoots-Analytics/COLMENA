...
class MPC_agent():
    ...
    def warm_start(self, state_horizon, dual_vars):
        for t in self.model.TimeHorizon:
            ...
            for area in self.model.other_areas:
                ...
                self.model.dual_vars[area, self.area, t] =  dual_vars.get((area, self.area, t),0)
                self.model.dual_vars[self.area, area, t] =   dual_vars.get((area, self.area, t),0)
...
class DynamicNeighbours(Context):
    @Dependencies(*["pyomo", "requests"])
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    
    def locate(self, device):
        agent_id = device.target_neighbor
        id = {'id':agent_id}
        print(json.dumps(id))

class AgentControl(Service):
    @Context(class_ref = GridAreas, name='grid_areas')
    @Context(class_ref = DynamicNeighbours, name='dynamic_neighbors')
    @Data(name = 'theta_data', scope = 'dynamic_neighbors/id = .')
    @Metric('frequency')
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
