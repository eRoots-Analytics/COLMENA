...
def setup_mpc(self, area, dt = 0.5, T = 20, controllable_redual = False):
    ...
    model.generators_bus = pyo.Set(initialize = generator_bus)
...
class DistributedMPC(Service):
    ...
    class LayerOne(Role):
        ...
        def first_warm_start(self, model):
            ...
            Sn_values = responseAndes.json()['value']
            responseAndes = requests.post(andes_url + '/area_variable_sync', json={'model':'PV', 'var':'pmax', 'area':self.area})
            pmax_values = responseAndes.json()['value']
            print(f"Pe_values are {Pe_values}")
            print(f"Pmax are {pmax_values}")
