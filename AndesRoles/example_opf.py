...
class AgentControl(Service):
    ...
    @Metric('frequency')
    @Data(name = "dual_vars_data", scope = ' ')
    @Data(name = 'Data_1', scope = ' ')
    @Data(name = 'Data_2', scope = ' ')
    @Data(name = 'Data_3', scope = ' ')
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    class LayerOne(Role):
        ...
        def behavior(self):
            ...
            while self.error > self.tol and self.iter < self.max_iter:
                ...
                self.state_horizon = json.loads(self.data_read.get().decode('utf-8')) 
                ...
                state_horizon = json.loads(self.data_read.get().decode('utf-8')) 
                for t in self.model.TimeHorizon:
                ...
                self.data_write.publish(state_horizon)
