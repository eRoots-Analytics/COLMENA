...
def add_area_mpc(self, model):
    ...
    model.delta_bus = pyo.Var(model.area_buses, model.TimeHorizon, bounds=(-0.1, 0.1))
...
def setup_mpc(self, area, dt = 0.5, T = 20, controllable_redual = False):
    ...
    def lagrangian_term(model):
        return sum( 10*sum(model.dual_lambda[i, t]*(model.delta_areas[i,t] - self.state_horizon_values[i,t]) for i in model.other_areas) for t in model.TimeHorizon)
    def penalty_term(model):
        return sum( 10*sum((model.delta_areas[i,t] - self.state_horizon_values[i,t])**2 for i in model.other_areas) for t in model.TimeHorizon)
...
class DistributedMPC(Service):
    ...
    class LayerOne(Role):
        ...
        def behavior(self):
            ...
            #We update the state horizon with the results
            for t in model.TimeHorizon:
                self.state_horizon_values[self.area, t] = model.delta[t].value
            ...
            error_index = max(delta_dual_vars)
            self.error.publish(max(delta_dual_vars.values()))
            print(f"dual vars are {self.dual_vars}")
            print(f"dual vars are {delta_dual_vars}")
            print(f"error is {error} for in dex {error_index}")
            print(f"iter is {self.iter}")
            self.first = False
            time.sleep(3)
            return 1
